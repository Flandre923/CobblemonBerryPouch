...
import com.gitlab.srcmc.rctmod.menu.SlotLocked;
...
public class BerryPouchContainer30 extends AbstractContainerMenu {
    ...
    public static BerryPouchContainer30 fromNetwork(int windowId, Inventory inv, FriendlyByteBuf buf) {
        InteractionHand hand = buf.readBoolean() ? InteractionHand.MAIN_HAND : InteractionHand.OFF_HAND;
        return new BerryPouchContainer30(windowId, inv, inv.player.getItemInHand(hand));
    }

    public BerryPouchContainer30(int windowId, Inventory playerInv, ItemStack bag) {
        super(ModRegistries.ModMenuTypes.BERRY_POUCH_CONTAINER_30.get(), windowId);
        this.bag = bag;

        if (!playerInv.player.level().isClientSide) {
            flowerBagInv = BerryPouch.getInventory(bag, playerInv.player.level());
        } else {
            flowerBagInv = new SimpleContainer(BerryPouch.MEDIUM_SIZE);
        }
    ...
    private void addBerrySlots(Container flowerBagInv) {
        ...
        for (int row = 0; row < 3; ++row) {
            for (int col = 0; col < 8; ++col) {
                ...
                addSlot(new Slot(flowerBagInv, slot, 11 + col * 18, 16 + row * 18) {
                    @Override
                    public boolean mayPlace(@NotNull ItemStack stack) {
                        return stack.is(BerryPouch.POUCH_GUI_30_STORAGE_SLOT.getSlotItem(slot+1));
                    }
                });
            }
        }
        ...
        for (int row = 0; row < 3; ++row) {
            for (int col = 0; col < 2; ++col) {
                ...
                addSlot(new Slot(flowerBagInv, slot, 163 + col * 18, 16 + row * 18) {
                    @Override
                    public boolean mayPlace(@NotNull ItemStack stack) {
                        return stack.is(BerryPouch.POUCH_GUI_30_STORAGE_SLOT.getSlotItem(slot+1));
                    }
    ...
    private void addInventorySlots(Inventory playerInv) {
        for (int row = 0; row < 3; ++row) {
            for (int col = 0; col < 9; ++col) {
                addSlot(new Slot(playerInv, col + row * 9 + 9, 25 + col * 18, 96 + row * 18));
            }
        }
    }
    ...
    private void addHotbarSlots(Inventory playerInv, ItemStack bag) {
        for (int i = 0; i < 9; ++i) {
            if (playerInv.getItem(i) == bag) {
                addSlot(new SlotLocked(playerInv, i, 25 + i * 18, 154));
            } else {
                addSlot(new Slot(playerInv, i, 25 + i * 18, 154));
            }
        }
    }
    ...
    public ItemStack quickMoveStack(Player player, int slotIndex) {
        ...
        if (clickedSlot != null && clickedSlot.hasItem()) { // 检查槽位和物品
            ...
            if (slotIndex < BerryPouch.MEDIUM_SIZE) { // 从花袋移动到玩家背包
                if (!this.moveItemStackTo(originalStack, BerryPouch.MEDIUM_SIZE, this.slots.size(), true)) { // 从花袋槽位后开始，到所有槽位结束，反向检查
                    return ItemStack.EMPTY; // 移动失败，返回空物品堆
                }
            } else { // 从玩家背包移动到花袋
                if (BerryPouch.POUCH_GUI_30_STORAGE_SLOT.has(originalStack.getItem())) { // 检查是否是允许放入花袋的物品
                    if (!this.moveItemStackTo(originalStack, 0, BerryPouch.MEDIUM_SIZE, false)) { // 从花袋的第一个槽位开始，到花袋槽位结束，正向检查
                        return ItemStack.EMPTY; // 移动失败，返回空物品堆
                    }
                } else {
