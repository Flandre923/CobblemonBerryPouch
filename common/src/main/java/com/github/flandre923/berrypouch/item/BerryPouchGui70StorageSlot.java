...
public class BerryPouchGui70StorageSlot extends AbstractBerryPouchStorageSlot {
    public BerryPouchGui70StorageSlot(int size) {
        super(size); // Pass the size to the parent constructor
    }

    @Override
    protected Item getSlotItemInternal(int slotIndex) {
        // Slot indices 1-70 correspond to the BerryPouchGui70_STORAGE_SLOTS
        switch (slotIndex) {
            ...
            case 70: return CobblemonItems.EGGANT_BERRY; // 添加第70个槽位
            default: return null; // Should not happen if slotIndex is valid (1-70)
        }
    }
}